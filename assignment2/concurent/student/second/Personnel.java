    ...
    public void startWar(List<Personnel> enemyArmy){
        ...
        synchronized(enemyArmy){
            if(health.get()>0 && enemyArmy.size()>0){
                ...
                opponent = enemyArmy.get(ThreadLocalRandom.current().nextInt(0, enemyArmy.size()));
             }
        }
    ...
    private void startAttacking(){
        ...
        synchronized(opponent){
            while(health.get()>0 && opponent.getHealth()>0){
                ...
                try {
                    Thread.sleep(ThreadLocalRandom.current().nextInt(ATTACK_WAIT_MINIMUM,ATTACK_WAIT_MAXIMUM));
